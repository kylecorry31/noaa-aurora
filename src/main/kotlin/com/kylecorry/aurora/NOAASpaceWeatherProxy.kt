...
class NOAASpaceWeatherProxy : ISpaceWeatherProxy {
    ...
    override suspend fun get3DayForecast(): List<DailySpaceWeatherForecast> {
        ...
        return kp
            ...
            .map { (date, kp) -> DailySpaceWeatherForecast(date, kp) }
    }


    override suspend fun getAlerts(): List<SpaceWeatherAlert> {
        ...
        return alerts.map {
            SpaceWeatherAlert(
                it.productId,
                ZonedDateTime.parse(it.issueDatetime.replace(" ", "T") + "Z"),
                it.message
            )
        }.sortedByDescending { it.issuedOn }
    }
    ...
    private fun getKpIndexForecast(
        ...
        val rows = forecastText.substring(tableStartIdx, tableEndIdx)
            .split("\n")
            .map { it.trim() }
